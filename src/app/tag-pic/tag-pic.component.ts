...
export class TagPicComponent implements OnInit {
  ...
  constructor(
    private getscope: GetScopeService, private sanitizer: DomSanitizer){
      this.model = {
        picurl: 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1599028409376&di=e4b60287b48c4f364045bbd99649e747&imgtype=0&src=http%3A%2F%2Fa1.att.hudong.com%2F05%2F00%2F01300000194285122188000535877.jpg',
        crane: '',
        lane: '',
        video: '',
        lorr: '',
  ...
  getImgbyHttp(): void
  {
    ...
    this.model.imgurls = [this.sanitizer.bypassSecurityTrustUrl(this.model.picurl)];
  }
  getImgbyPath(): void
  {
    ...
    this.model.imgurls = [].map.call(inputFile, file => this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(file)));
   ...
   clip(): void{
     ...
     for (let i = 0; i < picRefs.length; i++) {
      ...
      ctxpic.drawImage(image, 0, 0, image.width, image.height, 0, 0, image.width, image.height);
      // }
    // 画裁剪后的图片
    // image.onload=function(){
      ...
      ctx.rotate(angleRad);
      ctx.drawImage(piccanvas, x_min, y_min, xcenter, ycenter, 0, 0, xcenter, ycenter);
 ...
 // }
  ...
  setfanwei(): void
  {
    ...
    if ((this.model.lorr) === 'r'){this.model.lane = String(Number(this.model.lane) + 10); }
    console.log(this);
    console.log('getscope', this.getscope);
    this.getscope.getScope(this.model.crane, this.model.lane).subscribe(
      function(data): void {
        this.datae = data;
